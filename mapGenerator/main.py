...
def generate(name, width, height, mode, side_locked, detailed, scale, first_smoothing, smoothing, sharpness,
    ...
    for i in range(1, detailed + 1):
        ...
        for j in range(0, first_smoothing):
            smooth_queue = [(np.random.uniform(0, 1), x, y) for x in range(0, max_width)
    ...
    mount_level = water_level + 2 * (255 - water_level) / 3
    ...
    for y in range(0, height):
        for x in range(0, width):
            if side_locked:
                h_max = (height / 2)
                h = abs(y - h_max)
                pixel_heat = heatness - 3.0 * ((h / h_max) ** 2)
        ...
        for x in range(0, width):
            ...
            if pixel_h < water_level:
                if heat_map[x][y] * 20 > -2.0:
                    ...
                    blue = int(255 * bright)
                    colored_map[x][y] = (red, green, blue)
                    water_map.append((x, y))
    #Шаг 3. Разделение водоёмов
    print('Распознавание водоёмов')
    np.random.shuffle(water_map)
    sea_map = []
    for pos in water_map:
        if pos == (-3, -3):
            continue
        for p2 in water_map:
            if side_locked:
                if abs((p2[0] - pos[0]) % width) == 1:
                    if p2[1] == pos[1]
    exit(0)

    #Шаг 4. Отрисовка карты влажности
    ...
    iter_list = [(i, j) for i in range(0, width) for j in range(0, height)]
    ...
    for i in range(0, max_i):
        ...
        for o in iter_list:
            ...
            y = o[1]
            if itered_map[x][y] == 0:
                if humidity_map[x][y] > 0.05:
                    pos_for_iter.append(o)
                    itered_map[x][y] = 1
                else:
                    if humidity_map[x][y] < 0.0:
                        humidity_map[x][y] = 0.0
        iter_list.clear()
        ...
        for pos in pos_for_iter:
            ...
            if side_locked:
                if x > 0:
                    if itered_from_map[x][y][3] < humidity_map[x][y] * 0.9:
                    ...
                    if itered_from_map[x][y][3] < humidity_map[x][y] * 0.9:
                        if height_map[width - 1][y] * 255 >= water_level:
                            ...
                            sides += 1
                if x < width - 1:
                    if itered_from_map[x][y][1] < humidity_map[x][y] * 0.9:
                        if height_map[x + 1][y] * 255 >= water_level:
                    ...
                    if x < width - 1:
                        if itered_from_map[x][y][1] < humidity_map[x][y] * 0.9:
                            if height_map[x - 1][y] * 255 >= water_level:
                                ...
                                check_list.append((x + 1, y, 1))
            if y > 0:
                if itered_from_map[x][y][0] < humidity_map[x][y] - 0.05:
                    if height_map[x - 1][y] * 255 >= water_level:
                        sides += 1
                        check_list.append((x, y - 1, 0))
                if y < height - 1:
                    if itered_from_map[x][y][2] < humidity_map[x][y] * 0.9:
                        if height_map[x - 1][y] * 255 >= water_level:
                            sides += 1
            ...
            for p in check_list:
                ...
                if height_map[xx][yy] * 255 >= water_level:
                    ...
                    if heat_map[xx][yy] < 3.0:
                        raining += 0.02 * (3.0 - heat_map[xx][yy])
                result = value * (1 - raining)
                if height_map[xx][yy] * 255 > mount_level:
                    d = max(0.5, d)
                stopping = d ** 0.8
                result *= 1 - stopping
                result = (result + scale - 1) / scale
                if result > humidity_map[x][y] - 0.05:
                    result = humidity_map[x][y] - 0.05
                if result > itered_from_map[xx][yy][(dir + 2) % 4] - 0.05:
                    changed = False
                    if humidity_map[xx][yy] + result - itered_from_map[xx][yy][(dir + 2) % 4] <= humidity_map[x][y]:
                        ...
                        if humidity_map[xx][yy] < humidity_map[x][y]:
                            humidity_map[xx][yy] = humidity_map[x][y]
                            changed = True
                    if changed:
                        if itered_map[xx][yy] == 2:
                            itered_map[xx][yy] = 0
                            iter_list.append((xx, yy))
                        else:
                            if itered_map[xx][yy] == 0:
                                iter_list.append((xx, yy))
    ...
    for y in range(0, height):
        for x in range(0, width):
            ...
            if heat_map[x][y] * 20 <= -2.0:
                ...
                continue
            #Высокие горы
            if height_map[x][y] * 255 > water_level + 2 * (255 - water_level) / 3:
                bright = 0.6 + 0.4 * max(0.0, min(1.0, ((pixel_h - water_level) / (255 - water_level))))
                red = int(200 * bright)
                green = int(90 * bright)
                blue = int(0 * bright)
                colored_map[x][y] = (red, green, blue)
            ...
            if heat_map[x][y] * 20 < 12 and humidity_map[x][y] < 0.05:
                ...
                continue
            #Степь
            if heat_map[x][y] * 20 < 25 and 0.1 < humidity_map[x][y] < 0.3:
                bright = 0.6 + 0.4 * max(0.0, min(1.0, (pixel_h - water_level) / (255 - water_level)))
                red = int(180 * bright)
                green = int(255 * bright)
                blue = int(50 * bright)
...
if __name__ == '__main__':
    ...
    smoothing = 4
    sharpness = 5
    humidity = 0.6 #Влажность (процент от нормы)
    heatness = 2.0 #Жара (процент от нормы)
